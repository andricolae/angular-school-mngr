...
export const coursesReducer = createReducer(
  ...
  on(CourseActions.loadCoursesCount, (state) => ({ ...state, loading: true })),

  on(CourseActions.loadCoursesCountSuccess, (state, { coursesCount }) => ({
    ...state,
    coursesCount,
    loading: false,
  })),

  on(CourseActions.loadCoursesCountFail, (state, { error }) => ({
    ...state,
    error,
    loading: false,
  })),

  on(CourseActions.loadCoursesPage, (state) => ({ ...state, loading: true })),
  on(CourseActions.loadCoursesPageSuccess, (state, { courses }) => ({
    ...state,
    courses,
    loading: false,
  })),
  on(CourseActions.loadCoursesPageFail, (state, { error }) => ({
    ...state,
    loading: false,
    error,
  })),

  on(CourseActions.loadFilteredCoursesPage, (state) => ({
    ...state,
    loading: true,
  })),

  on(CourseActions.loadFilteredCoursesPageSuccess, (state, { courses }) => ({
    ...state,
    courses,
    loading: false,
  })),

  on(CourseActions.loadFilteredCoursesPageFail, (state, { error }) => ({
    ...state,
    error,
    loading: false,
  })),

  on(CourseActions.loadCoursesMaxPageIndex, (state) => ({
    ...state,
    loading: true,
  })),

  on(
    CourseActions.loadCoursesMaxPageIndexSuccess,
    (state, { maxPageIndex }) => ({
      ...state,
      maxPageIndex,
      loading: false,
  ...
  on(CourseActions.loadCoursesMaxPageIndexFail, (state, { error }) => ({
    ...state,
    error,
    loading: false,
  })),
  ...
  on(
    CourseActions.unenrollStudentSuccess,
    (state, { courseId, studentId }) => ({
      ...state,
      courses: state.courses.map((course) => {
        if (course.id === courseId) {
          const updatedCourse = {
  ...
  on(
    CourseActions.addStudentGradeSuccess,
    (state, { courseId, studentId, grade }) => {
      return {
        ...state,
        courses: state.courses.map((course) => {
          if (course.id === courseId) {
  ...
  on(
    CourseActions.updateStudentAttendanceSuccess,
    (state, { courseId, studentId, sessionId, present }) => {
      return {
        ...state,
        courses: state.courses.map((course) => {
          if (course.id === courseId) {
  ...
  on(
    CourseActions.checkScheduleStatusSuccess,
    (state, { courseId, isScheduled }) => {
      const updatedCourses = state.courses.map((course) => {
