...
export class CoursesEffects {
  ...
  loadCoursesPage$ = createEffect(() =>
    this.actions$.pipe(
      ofType(CourseActions.loadCoursesPage),
      tap(() =>
        this.logger.logAdmin('LOAD_COURSES_PAGE', 'Loading courses page')
      ),
      switchMap(({ direction }) =>
        this.courseService.getCoursesPage(direction).pipe(
          map(({ courses }) => {
            this.logger.logAdmin(
              'LOAD_COURSES_PAGE_SUCCESS',
              `Successfully loaded ${courses.length} courses page`
            );
            return CourseActions.loadCoursesPageSuccess({
              courses,
            });
          }),
          catchError((err) => {
            this.logger.logAdmin(
              'LOAD_COURSES_PAGE_FAIL',
              `Failed to load courses page: ${err.message}`,
              { error: err.message }
            );
            return of(
              CourseActions.loadCoursesPageFail({ error: err.message })
            );
          })
        )
      )
  ...
  loadFilteredCoursesPage$ = createEffect(() =>
    this.actions$.pipe(
      ofType(CourseActions.loadFilteredCoursesPage),
      tap(() =>
        this.logger.logAdmin('LOAD_COURSES_PAGE', 'Loading courses page')
      ),
      switchMap(({ filters, search, newFilter, direction }) =>
        this.courseService
          .getCoursesPageByFilter(filters, search, newFilter, direction)
          .pipe(
            map(({ courses }) => {
              this.logger.logAdmin(
                'LOAD_FILTERED_COURSES_PAGE_SUCCESS',
                `Successfully loaded ${courses.length} courses page`
              );
              return CourseActions.loadFilteredCoursesPageSuccess({
                courses,
              });
            }),
            catchError((err) => {
              this.logger.logAdmin(
                'LOAD_FILTERED_COURSES_PAGE_FAIL',
                `Failed to load courses page: ${err.message}`,
                { error: err.message }
              );
              return of(
                CourseActions.loadFilteredCoursesPageFail({
                  error: err.message,
                })
              );
            })
      ...
      mergeMap(({ course }) =>
        this.courseService.addCourse({ ...course }).pipe(
          map((courseId) => {
            this.logger.logCourse(
              'ADD_COURSE_SUCCESS',
              `Successfully added course: ${course.name}`,
              {
                courseId,
                courseName: course.name,
              }
          ...
          catchError((err) => {
            this.logger.logCourse(
              'ADD_COURSE_FAIL',
              `Failed to add course: ${err.message}`,
              {
                error: err.message,
                courseName: course.name,
              }
  ...
  loadCoursesMaxPageIndex$ = createEffect(() =>
    this.actions$.pipe(
      ofType(CourseActions.loadCoursesMaxPageIndex),
      mergeMap(({ filters, search }) =>
        from(this.courseService.getMaxPageIndex(filters, search)).pipe(
          map((maxPageIndex) =>
            CourseActions.loadCoursesMaxPageIndexSuccess({ maxPageIndex })
          ),
          catchError((error) =>
            of(CourseActions.loadCoursesMaxPageIndexFail({ error }))
          )
        )
      )
    )
  ...
  nextCoursesPage$ = createEffect(() =>
    this.actions$.pipe(
      ofType(CourseActions.nextCoursesPage),
      map(() => {
        return CourseActions.loadCoursesPage({
          direction: 'next',
        });
      })
    )
  ...
  previousCoursesPage$ = createEffect(() =>
    this.actions$.pipe(
      ofType(CourseActions.previousCoursesPage),
      map(() => {
        return CourseActions.loadCoursesPage({
          direction: 'prev',
        });
  ...
  nextFilteredCoursesPage$ = createEffect(() =>
    this.actions$.pipe(
      ofType(CourseActions.nextFilteredCoursesPage),
      map(({ filters, search, newFilter }) => {
        return CourseActions.loadFilteredCoursesPage({
          filters,
          search,
          newFilter,
          direction: 'next',
  ...
  previousFilteredCoursesPage$ = createEffect(() =>
    this.actions$.pipe(
      ofType(CourseActions.previousFilteredCoursesPage),
      map(({ filters, search, newFilter }) => {
        return CourseActions.loadFilteredCoursesPage({
          filters,
          search,
  ...
  loadCoursesCount$ = createEffect(() =>
    this.actions$.pipe(
      ofType(CourseActions.loadCoursesCount),
      switchMap(() =>
        from(this.courseService.getCoursesCount()).pipe(
          map((coursesCount) =>
            CourseActions.loadCoursesCountSuccess({ coursesCount })
          ),
          catchError((err) =>
            of(CourseActions.loadCoursesCountFail({ error: err.message }))
          )
        )
      )
    )
        ...
        this.courseService.deleteCourse(courseId).pipe(
          map(() => {
            this.logger.logCourse(
              ...
              `Successfully deleted course with ID: ${courseId}`,
              { courseId }
            );
            NotificationComponent.show(
              'success',
        ...
        this.courseService.updateCourse(course).pipe(
          map(() => {
            this.logger.logCourse(
              ...
              `Successfully updated course: ${course.name}`,
              {
                courseId: course.id,
                courseName: course.name,
              }
  ...
  unenrollStudent$ = createEffect(() =>
    this.actions$.pipe(
      ...
      tap(({ courseId, studentId }) =>
        this.logger.logStudent(
          'UNENROLL_STUDENT',
          `Unenrolling student from course`,
          {
            courseId,
            studentId,
          }
      ...
      mergeMap(({ courseId, studentId }) =>
        this.courseService.unenrollStudent(courseId, studentId).pipe(
          map(() => {
            ...
            return CourseActions.unenrollStudentSuccess({
              courseId,
              studentId,
            });
          }),
          catchError((error) => {
            this.logger.logStudent(
              ...
              `Failed to unenroll student from course: ${error.message}`,
              {
                courseId,
                studentId,
                error: error.message,
            ...
            NotificationComponent.show(
              'alert',
              `Failed to unenroll from course: ${error.message}`
            );
            return of(
              CourseActions.unenrollStudentFail({ error: error.message })
            );
          })
        )
      )
  ...
  markCourseForScheduling$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      mergeMap(({ courseId }) =>
        this.http
          .post<any>(
            'https://school-api-server.vercel.app/api/mark-course-pending/',
            { courseId }
          )
          .pipe(
            map(() => {
              this.logger.logSchedule(
                'MARK_FOR_SCHEDULING_SUCCESS',
                `Successfully marked course for scheduling`,
                { courseId }
              );
            ...
            catchError((error) => {
              this.logger.logSchedule(
                'MARK_FOR_SCHEDULING_FAIL',
                `Failed to mark course for scheduling: ${error.message}`,
                {
                  courseId,
                  error: error.message,
                }
              ...
              return of(
                CourseActions.markCourseForSchedulingFailure({ error })
              );
            }),
            tap(() => {
              ...
              NotificationComponent.show(
                'success',
                'Course successfully marked for scheduling'
              );
            })
          )
      )
    ...
    this.actions$.pipe(
      ...
      tap(({ courseId }) =>
        this.logger.logSchedule(
          ...
          `Checking course schedule status`,
          { courseId }
        )
      ),
      mergeMap(({ courseId }) =>
        this.courseService.checkScheduleStatus(courseId).pipe(
          map((isScheduled) => {
            this.logger.logSchedule(
              'CHECK_SCHEDULE_STATUS_SUCCESS',
              `Successfully checked course schedule status: ${
            ...
            return CourseActions.checkScheduleStatusSuccess({
              courseId,
              isScheduled,
            });
          }),
          catchError((error) => {
            this.logger.logSchedule(
              'CHECK_SCHEDULE_STATUS_FAIL',
              `Failed to check course schedule status: ${error.message}`,
              {
                courseId,
                error: error.message,
              }
            ...
            return of(
              CourseActions.checkScheduleStatusFail({ error: error.message })
            );
          })
        )
      )
            ...
            return this.store.select(UserSelectors.selectAllUsers).pipe(
              ...
              mergeMap((users) => {
                ...
                return this.courseService
                  .addStudentGrade(courseId, studentId, grade)
                  .pipe(
                    map((gradeId) => {
                      this.logger.logGrade(
                        'ADD_GRADE_SUCCESS',
                    ...
                    catchError((error) => {
                      ...
                      NotificationComponent.show(
                        'alert',
                        `Failed to add grade: ${error.message}`
                      );
                      return of(
                        CourseActions.addStudentGradeFail({
                          error: error.message,
                        })
                      );
                    })
          ...
          mergeMap((course) => {
            return this.store.select(UserSelectors.selectAllUsers).pipe(
              ...
              mergeMap((users) => {
                ...
                this.logger.logAttendance(
                  'UPDATE_ATTENDANCE',
                  `${
                    present ? 'Marking' : 'Removing'
                  } attendance for student "${studentName}" in course "${courseName}" for session on ${sessionDate} at ${sessionTime}`,
                  {
                    courseId,
                    courseName,
                    studentId,
                  ...
                  .pipe(
                    map(() => {
                      this.logger.logAttendance(
                        ...
                        } student "${studentName}" in course "${courseName}" for session on ${sessionDate}`,
                        {
                          courseId,
                          courseName,
                          studentId,
                    ...
                    catchError((error) => {
                      ...
                      return of(
                        CourseActions.updateStudentAttendanceFail({
                          error: error.message,
                        })
                      );
                    })
                ...
                return this.courseService
                  ...
                  .pipe(
                    map(() => {
                      ...
                      return CourseActions.enrollStudentSuccess({
                        courseId,
                        studentId,
                      });
                    }),
                    catchError((error) => {
                      ...
                      return of(
                        CourseActions.enrollStudentFail({
                          error: error.message,
                        })
                      );
                    })
      ...
      mergeMap(({ courseId, studentId }) =>
        this.courseService.removePendingStudent(courseId, studentId).pipe(
          map(() => {
            ...
            NotificationComponent.show(
              'success',
              'Pending student removed successfully'
            );
            return CourseActions.removePendingStudentSuccess({
              courseId,
              studentId,
            });
          })
      ...
      mergeMap(({ courseId, studentId }) =>
        this.courseService.acceptPendingStudent(courseId, studentId).pipe(
          map(() => {
            ...
            NotificationComponent.show(
              'success',
              'Pending student accepted successfully'
            );
            return CourseActions.acceptPendingStudentSuccess({
              courseId,
              studentId,
            });
          })
